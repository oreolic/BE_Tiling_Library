...
def ngg_patterns(sa_ngg,cds_extended,codon_start,trimmed_cds,pamtype,window_start=3,window_end=7):
    ...
    for pam in pamlst:
        ...
        if len(editable_codon_index) == 0:
            ...
            loc_gx20 = cds_extended.find(gx20)
            ## target: 4+20+3+3 

            target = cds_extended[loc_gx20-4:loc_gx20+20+3+3]
...
def ccn_patterns(sa_ccn,cds_extended,codon_start,trimmed_cds,pamtype,window_start=3,window_end=7):
    ...
    for pam in pamlst:
        ...
        if len(editable_codon_index) == 0:
            ...
            ## deep input = 24 bp target sequence (1 bp + 20 bp protospacer + PAM + 3 bp) 
            deep_input = cds_extended[loc_gx20-6:loc_gx20+20+1]

            ## target: 4+20+3+3
            target = cds_extended[loc_gx20-6:loc_gx20+20+4]
            
            target = General().rt_sequence(target)
            eachdf['Target'] = target
            eachdf['DeepInput'] = deep_input
...
def pe_editable_pattern(eachdf,pamtype,window_start,window_end):
    ...
    for var_num in final_dic:
        eachvar = final_dic[var_num]
        if eachvar.shape[0] == 0:    
            return pd.DataFrame()
